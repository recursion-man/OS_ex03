...
void printStatsToHeaders(Node *request, char * buf)
{
...
void requestError(Node *request, char *cause, char *errnum, char *shortmsg, char *longmsg)
{
   ...
   printf("%s", buf);
    ...
    printStatsToHeaders(request, buf);
    sprintf(buf, "%s\r\n", buf);
...
void requestServeDynamic(Node* request, char *filename, char *cgiargs)
{
   char buf[MAXLINE], *emptylist[] = {NULL};

   request->stats.thread_dynamic++;
...
void requestServeStatic(Node* request , char *filename, int filesize)
{
   int srcfd;
   ...
   Rio_writen(request->fd, srcp, filesize);
   Munmap(srcp, filesize);

}

// handle a request
void requestHandle(Node* request)
{
   int fd = request->fd;
   int is_static;
   struct stat sbuf;
